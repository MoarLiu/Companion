...
if [[ "${COMPANION_AUTO_BUMP_BUILD:-${COMPANION_AUTO_BUMP_BUILD:-1}}" != "0" ]]; then
...
CODE_SIGN_IDENTITY="${CODE_SIGN_IDENTITY:-${COMPANION_CODE_SIGN_IDENTITY:-${COMPANION_CODE_SIGN_IDENTITY:--}}}"
NOTARIZE="${NOTARIZE:-0}"
NOTARY_PROFILE="${NOTARY_PROFILE:-${COMPANION_NOTARY_PROFILE:-${COMPANION_NOTARY_PROFILE:-}}}"
...
if [[ "$CODE_SIGN_IDENTITY" != "-" ]]; then
  ...
  if [[ "$NOTARIZE" == "1" ]]; then
    if [[ -z "$NOTARY_PROFILE" ]]; then
      echo "NOTARY_PROFILE or COMPANION_NOTARY_PROFILE is required when NOTARIZE=1." >&2
      exit 1
    fi
