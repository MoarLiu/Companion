...
IDENTITY_NAME="${COMPANION_CODE_SIGN_IDENTITY:-Companion Local Development Code Signing}"
KEYCHAIN="${COMPANION_CODE_SIGN_KEYCHAIN:-$(/usr/bin/security default-keychain | sed -E 's/^[[:space:]]*"//; s/"[[:space:]]*$//')}"
CERT_DAYS="${COMPANION_CODE_SIGN_CERT_DAYS:-3650}"
...
/usr/bin/openssl req \
  ...
  -subj "/CN=$IDENTITY_NAME/O=Companion Local Development/OU=Local Development" \
