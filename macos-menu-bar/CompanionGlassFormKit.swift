...
// 统一的玻璃风格表单弹窗工具集。
// 让配置类弹窗复用与「日志 / 提醒事项」一致的 SwiftUI 玻璃外观:
// 复用 CompanionModalPanelStyle 的玻璃外壳 + runModal 流程,内容则是 SwiftUI。
...
enum CompanionGlassModalHost {
    ...
    static func runModal<Content: View>(
        ...
        let hostingView = CompanionModalHostingView(rootView: content())
        ...
        CompanionModalPanelStyle.installModalCancelHandler({
...
private final class CompanionModalHostingView<Content: View>: CompanionInteractiveHostingView<Content> {
...
struct CompanionModalHeader: View {
    var icon: String
    var tint: Color = XiaoHuaErTheme.tint
    var title: String
    var message: String?
...
/// 表单字段上方的小标题。
struct CompanionModalFieldLabel: View {
    var text: String
...
extension View {
    ...
    func companionModalFieldChrome(height: CGFloat = 40, radius: CGFloat = 12) -> some View {
        self
            ...
            .overlay(
                RoundedRectangle(cornerRadius: radius, style: .continuous)
                    .stroke(XiaoHuaErTheme.tint.opacity(0.22), lineWidth: 1)
            )
    }

    /// 弹窗整体外框:统一内边距 + 玻璃面板背景。
    /// 高度交给内容自然撑开(配合 CompanionGlassModalHost 的合身测量),
    /// 因此这里不用 maxHeight: .infinity,避免底部留白。
    func companionModalFormBody() -> some View {
        self
            .padding(.top, 24)
